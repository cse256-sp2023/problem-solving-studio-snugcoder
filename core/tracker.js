...
let $topBanner = $("#mturk-top-banner"),
    ...
    $textArea = $("#text-area"),
    $document = $(document),
    log = [new SpecialEventEntry(ActionEnum.SPECIAL_EVENT, new Date().getTime(), { purpose: "This is the first event." })],
...
function setUpInitial() {
    ...
    function attachInitialListeners() {
        $backButton.on("click", function(e) {
            ...
            if (response.success) {
                log.push(new SpecialEventEntry(ActionEnum.BACK, new Date().getTime(), response.url));
        ...
        $('#scenario_input').on("change", function(e) { userData.taskData.response = e.target.value });

        $htmlLoc.on("click", function(e) {
            if (e.originalEvent.logged === undefined) {
                const le = new ClickEntry(ActionEnum.CLICK, (e.clientX + window.pageXOffset), (e.clientY + window.pageYOffset), e.target.id, new Date().getTime());
                log.push(le);
        ...
        $document.keypress(function(e) {
            if (e.target.id !== "scenario_input") {
                let le = new ButtonDownEntry(ActionEnum.BUTTON_DOWN, mousePosition.x, mousePosition.y, e.target.id, e.key, new Date().getTime());
                log.push(le);
            }
        });
        let lastTop = { x: 0, y: 0 };
        $document.scroll(function(e) {
            ...
            if (Math.abs(lastTop.x - dx) > 10 || Math.abs(lastTop.y - dy) > 10) {
                lastTop.x = dx;
                lastTop.y = dy;
                lastItem = log[log.length - 1];
                if (lastItem && lastItem.action && lastItem.action === ActionEnum.SCROLL && (dtime - lastItem.time <= 20)) {
                    lastItem.time = dtime;
                    lastItem.x = dx;
                    lastItem.y = dy;
                } else {
...
function attachLinkListeners() {
    $("#html-loc a").on("click",
        ...
        function(e) {
            ...
            const nextURL = pageHistory.forwardHistory(new URL(e.target.href).pathname);
            e.preventDefault();
            if (!(e.target.classList.contains("download"))) {
                standardLoad(nextURL);
            } else {
                let elem = e.target;
                if (elem.nextElementSibling && elem.nextElementSibling.dataset.attachment && elem.nextElementSibling.dataset.attachment === "true") {
                    //pass
                } else {
                    let df = document.createRange().createContextualFragment(`<br data-attachment="true"><iframe width="100%" height="400" src="${e.target.href}"></iframe>`);
...
function attachImageListeners() {
    $("#html-loc img").on("click", function(e) {
        ...
        standardLoad(pageURL, () => { $("#single-image").attr({ 'src': imageURL }); })
    });
}
...
function attachFormListener2() {
    ...
    let onError = (message, source, lineno, colno, error) => {
        const eMessage = error === null || error === undefined || error.message === null ? "NA" : error.message;
        let newVal = $debugField.val() + '_0_0_' + eMessage + '_0_0_' + message + '_0_0_';
    ...
    let submitFunc = (e) => {
        ...
        userData.taskData.response = {
            ...
            task_response_complications: all_the_data.task_response_complications,
        ...
        $mturkSubmitForm.removeEventListener(
            'submit',
